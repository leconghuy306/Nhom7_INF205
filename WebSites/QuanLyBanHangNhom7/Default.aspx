	...
	<div class="content">
		<div class="container">
			<div class="row">
			    ...
			    <asp:SqlDataSource ID="QLBH" runat="server" ConnectionString="<%$ ConnectionStrings:ConnectionString %>" DeleteCommand="DELETE FROM [product] WHERE [product_id] = @product_id" InsertCommand="INSERT INTO [product] ([product_id], [productcategory_id], [sku], [names], [price], [discription], [img]) VALUES (@product_id, @productcategory_id, @sku, @names, @price, @discription, @img)" SelectCommand="SELECT [product_id], [productcategory_id], [sku], [names], [price], [discription], [img] FROM [product]" UpdateCommand="UPDATE [product] SET [productcategory_id] = @productcategory_id, [sku] = @sku, [names] = @names, [price] = @price, [discription] = @discription, [img] = @img WHERE [product_id] = @product_id" ProviderName="System.Data.SqlClient">
                    <DeleteParameters>
